...
FROM 
    ipl_bidding_details bdg 
INNER JOIN 
    ipl_bidder_details bdr ON bdg.BIDDER_ID = bdr.BIDDER_ID
GROUP BY 
    bidder_id
ORDER BY 
    win_percentage DESC;
...
SELECT
    ...
    ips.STADIUM_NAME,
    ips.CITY,
    COUNT(im.MATCH_ID) AS NO_OF_MATCHES
FROM
    ipl_stadium ips
LEFT JOIN
    ipl_match_schedule im
ON
    ips.STADIUM_ID = im.STADIUM_ID
GROUP BY
    ips.STADIUM_ID,
    ips.STADIUM_NAME,
    ips.CITY
...
FROM
    ipl_match im
INNER JOIN
    ipl_match_schedule ims ON ims.MATCH_ID = im.MATCH_ID
INNER JOIN
    ipl_stadium s ON s.STADIUM_ID = ims.STADIUM_ID
GROUP BY
    s.stadium_name
ORDER BY
    win_percentage DESC;
...
-- Conclusion:  This SQL query retrieves the total bids along with the bid team and team name.
--               It joins the ipl_bidding_details table with the ipl_team table based on the team ID present in both tables.
...
-- Problem Statement:06  Display total matches played, total matches won and total matches lost by the team along with its team name.
...
WHERE 
    TS.team_id IN (1, 2, 3, 4, 5, 7, 8)
GROUP BY 
    TS.team_id, T.Team_name;
    
-- Conclusion: This SQL query retrieves the total matches played, total matches won, and total matches lost by each team, along with its team name.
--              It aggregates data from the ipl_match table, using conditional aggregation to calculate matches won and lost.
...
INNER JOIN 
    ipl_team tea ON play.TEAM_ID = tea.TEAM_ID
INNER JOIN 
    ipl_player player ON play.PLAYER_ID = player.PLAYER_ID
WHERE 
    PLAYER_ROLE LIKE '%Bowler%' 
    AND tea.REMARKS LIKE '%MI%';
...
-- Conclusion:  This SQL query retrieves the teams with more than 4 all-rounders in descending order of the count of all-rounders.
...
INNER JOIN 
    ipl_bidder_points tp ON bd.BIDDER_ID = tp.BIDDER_ID
INNER JOIN 
    ipl_match_schedule sd ON bd.SCHEDULE_ID = sd.SCHEDULE_ID
INNER JOIN 
    ipl_match mt ON sd.MATCH_ID = mt.MATCH_ID
INNER JOIN 
    ipl_team tm ON mt.MATCH_WINNER = tm.TEAM_ID
INNER JOIN 
    ipl_stadium std ON sd.STADIUM_ID = std.STADIUM_ID
WHERE 
    tm.REMARKS = 'CSK'
    AND mt.TEAM_ID1 = mt.MATCH_WINNER
    AND std.STADIUM_NAME LIKE '%Chinnaswamy %';

-- Conclusion: This SQL query retrieves the total bidders' points for each bidding status of those bidders who bid on CSK when it won the match in M. Chinnaswamy Stadium, year-wise.
--             It filters data based on the conditions provided in the problem statement.
...
SELECT * FROM 
    (SELECT *, DENSE_RANK() OVER (ORDER BY WICKETS DESC) AS PLAYER_RANK 
     FROM 
        (SELECT it.TEAM_NAME, a.PLAYER_NAME, b.PLAYER_ROLE,
            CAST(SUBSTRING_INDEX(SUBSTRING(performance_dtls, LOCATE("Wkt", performance_dtls), 6), "-", -1) AS SIGNED) AS WICKETS
         FROM ipl_player a
...
-- Conclusion:  This SQL query extracts the top 5 bowlers and all-rounders based on the number of wickets they have taken.
--             It parses the performance details column to extract the wicket count for each player.
...
FROM 
    ipl_match mt 
INNER JOIN 
    ipl_match_schedule sdl ON mt.MATCH_ID = sdl.MATCH_ID
INNER JOIN 
    ipl_bidding_details bd ON sdl.SCHEDULE_ID = bd.SCHEDULE_ID
INNER JOIN 
    ipl_bidder_details bt ON bd.BIDDER_ID = bt.BIDDER_ID
INNER JOIN 
    ipl_bidder_points pt ON bt.BIDDER_ID = pt.BIDDER_ID
GROUP BY 
    bt.BIDDER_ID, bt.BIDDER_NAME
ORDER BY 
    Toss_Winning_Percent DESC;

-- Conclusion:   This SQL query retrieves the percentage of toss wins for each bidder and displays the results in descending order based on the percentage.
--               It calculates the toss win percentage by counting the number of times a bidder's team won the toss and dividing it by the total number of matches played by the bidder.
...
SELECT 
    bdr.BIDDER_ID,
    bdr.BIDDER_NAME, 
    YEAR(bdg.BID_DATE) AS Year, 
    MONTH(bdg.BID_DATE) AS Month, 
    SUM(pts.TOTAL_POINTS) AS Total_Points
FROM 
    ipl_bidder_details bdr
INNER JOIN 
    ipl_bidder_points pts ON bdr.BIDDER_ID = pts.BIDDER_ID 
INNER JOIN 
    ipl_bidding_details bdg ON pts.BIDDER_ID = bdg.BIDDER_ID
WHERE 
    YEAR(bdg.BID_DATE) = 2017
GROUP BY 
    bdr.BIDDER_ID,
    bdr.BIDDER_NAME,
    YEAR(bdg.BID_DATE),
...
-- Conclusion:This SQL query retrieves data to calculate the total points month-wise for the 2017 bid year.
--           It joins the bidder details, bidder points, and bidding details tables based on the bidder ID.
--            The data is filtered for bids made in the year 2017.
    ...
    (
        SELECT
            SUM(CASE WHEN BD1.Bid_Status = 'Won' THEN 2
                     WHEN BD1.Bid_Status = 'Lost' THEN 1
                     ELSE 0 END)
        FROM
...
-- Conclusion: This SQL query retrieves data to calculate the total points month-wise for the 2017 bid year using subqueries.
--             It calculates the total points for each bidder using a subquery within the SELECT clause.
--              The data is filtered for bids made in the year 2017.
...
-- Conclusion: This SQL query retrieves the top 3 and bottom 3 bidders based on the total bidding points for the year 2018.
